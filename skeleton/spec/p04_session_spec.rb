...
require 'session/controller_base'
...
describe Session::ControllerBase do
  before(:all) do
    class CatsController < Session::ControllerBase
    end
  end
  ...
  describe "#session" do
    it "returns a session instance" do
      expect(cats_controller.session).to be_a(Session::Session)
    end
